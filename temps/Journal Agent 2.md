...
### Tunnel US « Statistiques des scores » — revue, BDD, TDD, Designer

#### Prompt
Enchaîner tout le tunnel pour l’US « Statistiques des scores » (histogramme des scores sous le tableau de bord) sans validation utilisateur : revue du livrable, BDD manquants, TDD back-end (API histogramme), TDD front-end (tests IHM), Designer (CSS). Rapport final et journal.

#### Résultat
- **Revue** : Livrable vérifié — API GET `/api/histogramme-scores-offres`, layout 3 colonnes (traitements | histogramme | consommation), section centre « Statistiques des scores », bouton Calculer, canvas 10 barres, population Score_Total ≠ 0 ou Statut = Expiré. `npm run lint` absent du projet ; `npm run typecheck` OK. DoD conforme.
- **BDD** : Fichier `tests/bdd/statistiques-scores-offres.feature` créé (bloc visible, titre/intro/canvas/bouton, API 10 buckets + total, API sans config → ok false). Steps dans `statistiques-scores-offres.steps.ts` ; step « container intitulé Statistiques des scores » ajouté dans `parametrage-ia.steps.ts`.
- **TDD-back-end** : `app/api-handlers.histogramme-scores.test.ts` — 3 tests : config absente → 200 ok false buckets vide ; config présente + mock fetch → 10 buckets et total ; population (exclusion Score_Total 0 et non Expiré). Tous passent.
- **TDD-front-end** : Dans `app/layout-html.test.ts`, 2 tests ajoutés : bloc Statistiques des scores (titre, intro, canvas, bouton, aria-label) ; script appelle `/api/histogramme-scores-offres` au clic.
- **Designer** : `.boutonCalculerHistogrammeScores` ajouté aux sélecteurs btnSecondary (base, hover, focus) et au media query dans `app/content-styles.css` pour cohérence avec les autres boutons secondaires du tableau de bord.
- **Tests** : 469 tests unitaires passent (dont 3 histogramme-scores, 35 layout-html). **Done.**
