/**
 * Lecture / écriture de la section ClaudeCode dans data/parametres.json (US-2.2).
 * API Key stockée chiffrée (apiKeyChiffre), jamais en clair dans le fichier.
 */
import type { ClaudeCode, ClaudeCodeLu } from '../types/parametres.js';
import {
  lireParametres,
  ecrireParametres,
...
/**
 * Retourne la section claudecode avec apiKey déchiffrée (pour usage serveur). Ne pas exposer apiKey au client.
 * hasApiKey permet à l'app de savoir si une clé est configurée sans en exposer la valeur.
 */
export function lireClaudeCode(dataDir: string): ClaudeCodeLu | null {
  const p = lireParametres(dataDir);
  const raw = p?.claudecode;
  if (!raw) return null;
...
/**
 * Met à jour la section claudecode (fusion partielle). Si updates.apiKey est fourni, il est chiffré avant stockage.
 * Ne jamais écrire apiKey en clair dans le fichier.
 */
export function ecrireClaudeCode(dataDir: string, updates: Partial<ClaudeCode>): void {
  const p = lireParametres(dataDir) ?? getDefaultParametres();
  const existing = p.claudecode ?? {};
