 ...
 * En CI, ne pas les définir → le test s’exécute sans connexion réelle.
...
describe('Connecteur IMAP - intégration (US-3.17)', () => {
  it('se connecte au serveur IMAP et retourne le nombre d\'emails du dossier', async () => {
    if (!hasImapTestEnv) {
      return; // Pas d’env IMAP : test exécuté mais sans connexion réelle
    }
