#### US-1.12 : Analyse des emails Cadre Emploi

- **En tant que** utilisateur
- **Je souhaite** que les offres contenues dans les emails Cadre Emploi soient detectees, creees puis enrichies via l'architecture plugin en 2 etapes
...
- **CA1 - Audit de la source Cadre Emploi** :
  - Le matching de l'expediteur se fait en comparaison exacte insensible a la casse.
  - Un expediteur proche mais non exact (exemple : variante avec +alias) n'est pas reconnu comme Cadre Emploi.
  - Si la source n'existe pas, elle est creee avec `plugin = "Cadre Emploi"` et `actif = true`.
  - Si la source existe deja, elle est mise a jour pour garantir `plugin = "Cadre Emploi"` et `actif = true`.

- **CA2 - Etape 1 plugin email (parsing et creation des offres)** :
  - Le plugin email Cadre Emploi est branche dans le registry de plugins de l'etape 1.
...
- **CA3 - Etape 2 plugin fetch (recuperation et enrichissement)** :
  - Le plugin fetch Cadre Emploi est branche dans le mecanisme d'enrichissement de l'etape 2.
