...
- Le BDD exige explicitement une source HelloWork active :
  - `tests/bdd/offres-emails-hellowork.feature` (CA1/CA2 : algo `HelloWork`, `actif=true`).
- Le code métier audit/source continue de mapper uniquement `Linkedin` ou `Inconnu` :
  - `utils/gouvernance-sources-emails.ts` (`AlgoSource = 'Linkedin' | 'Inconnu'`, mapping expéditeur sans cas HelloWork).
  - `scripts/run-audit-sources.ts` (`algoParExpediteur`: LinkedIn sinon Inconnu ; activation par défaut seulement LinkedIn).
...
- Impact : dette technique + incohérence du domaine (type `AlgoSource` non aligné).
...
- Attendu minimum pour nouvelle livraison :
  ...
  - Supprimer les casts `as never` en modélisant l’algo proprement,
