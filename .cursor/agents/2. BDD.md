...
- **Une seule US à la fois** : tu sais **implicitement** quelle US traiter en lisant le fichier **`.cursor/sprints/US en cours.md`** (ligne 1 = référence, ligne 2 = titre, **ligne 3 = étape** : `US` | `BDD` | `TDD-back-end` | `TDD-front-end` | `Designer`).
...
- Tu es invoqué par le **Lead Dev** lorsque l'étape (ligne 3 de `.cursor/sprints/US en cours.md`) est **`BDD`**. Tu lis l'US en cours (lignes 1–2) pour savoir quelle US traiter. Si aucune US en cours → alerte, erreur. Sinon, tu produis uniquement des fichiers `.feature` dans `.\tests\bdd`, sans code.
