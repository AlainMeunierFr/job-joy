...
- Tu es invoqué par le **Lead Dev** lorsque l'étape (ligne 3 de `.cursor/sprints/US en cours.md`) est **`TDD-back-end`**. Tu lis l'US en cours pour savoir quelle US implémenter. Si aucune US en cours → alerte, erreur. Sinon, tu codes uniquement dans `.\utils` (et `.\types` si pertinent), en TDD baby steps.
