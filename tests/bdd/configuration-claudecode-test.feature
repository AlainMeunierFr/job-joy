# language: fr
@us-2.4
Fonctionnalité: Configuration ClaudeCode - Test
  En tant qu'utilisateur, je souhaite vérifier que les paramétrages de l'API ClaudeCode sont corrects,
  ...
  Scénario: La section Configuration ClaudeCode propose un champ Texte d'offre à tester
  ...
  Scénario: La section Configuration ClaudeCode comporte un bouton Tester API
    Alors la section Configuration ClaudeCode comporte un bouton "Tester API"

  Scénario: Au clic sur Tester API avec erreur API, le code erreur est affiché
    Étant donné que l'API ClaudeCode est mockée pour renvoyer une erreur avec un code connu
  ...
  Scénario: Au clic sur Tester API avec succès, le résultat est affiché de manière lisible
    Étant donné que l'API ClaudeCode est mockée pour renvoyer un résultat d'analyse valide
    Et que le champ "Texte d'offre à tester" contient un texte
    Et que je suis sur la page Paramètres
    Quand je clique sur le bouton "Tester API" de la section Configuration ClaudeCode
  ...
  # --- CA4 : Tous les éléments dans la section Configuration ClaudeCode ---
  Scénario: Les éléments de test (champ texte, boutons Récupérer et Tester API) sont dans la section Configuration ClaudeCode
    Étant donné qu'au moins une offre Airtable est disponible en base
    Quand je me rends sur la page Paramètres
    Alors la page comporte un container ou une section intitulée "Configuration ClaudeCode"
    Et la section Configuration ClaudeCode comporte un champ ou une zone intitulée "Texte d'offre à tester"
